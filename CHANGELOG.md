...
* New Features:
  - **Quests can ask questions, answered as part of the submission.** Open a quest and use **Manage Questions** (on the quest edit form beside Edit Prerequisites, and on the quest's detail page) to add as many questions as you like, of three kinds: a **short answer** (one line, up to 200 characters), a **long answer** (the full editor, with formatting and images), and a **file upload** (which can be restricted to images, video, audio, image or video, or any file). Each question takes instructions the student reads, an optional **solution** and **marker notes** that only staff ever see, and a Required flag; the up and down arrows put them in the order students answer them. A student then fills in one field per question above the usual comment box when they submit, with their text saved as they type, and their answers arrive with the submission for marking, each beside that question's solution and marker notes so you can compare without leaving the page. A photo, a video or an audio recording is shown or played right there rather than being a file to download, and a quest with no questions behaves exactly as it always has. Copying a quest copies its questions [#1304](https://github.com/bytedeck/bytedeck/issues/1304) [#2161](https://github.com/bytedeck/bytedeck/issues/2161) [#2172](https://github.com/bytedeck/bytedeck/issues/2172) [#2349](https://github.com/bytedeck/bytedeck/issues/2349)
* Tweaks:
  - Reordering a quest's questions no longer reloads the page, so a long list stays where you were reading it instead of jumping back to the top on every click [#2216](https://github.com/bytedeck/bytedeck/issues/2216)
  - A short answer now tells the student it takes up to 200 characters, rather than silently stopping their typing at the limit [#2401](https://github.com/bytedeck/bytedeck/issues/2401)
  - The submission form's answer editors are shorter, so a quest asking several long answers no longer buries its submit button; the question table's tooltips read as the teacher typed them, and its file-type help sits on one line [#2169](https://github.com/bytedeck/bytedeck/issues/2169)
* Bugfixes:
  - A quest shared to the Shared Library arrived on the importing deck without any of its questions, so a quest whose instructions said "answer the questions below" became an empty-form quest, and nothing said so; questions now travel with the quest [#2162](https://github.com/bytedeck/bytedeck/issues/2162)
  - The Library's import preview listed the questions of whichever quest on your own deck happened to share an id with the one being previewed, and offered to edit or delete them; it now shows the questions of the quest you are importing, and drops the management buttons [#2163](https://github.com/bytedeck/bytedeck/issues/2163)
  - Answering a question and then hitting a validation error (a missing required answer, an oversized file) discarded the files and attachments you had already chosen, so you had to pick them again; they are kept now [#2165](https://github.com/bytedeck/bytedeck/issues/2165) [#2427](https://github.com/bytedeck/bytedeck/issues/2427)
  - Skipping a quest left the student's half-finished answers behind, where they reappeared on the completed submission; a skip now clears them and keeps the comment the student was drafting [#2164](https://github.com/bytedeck/bytedeck/issues/2164) [#2431](https://github.com/bytedeck/bytedeck/issues/2431)
  - A student could complete another student's submission by posting to its URL directly [#2167](https://github.com/bytedeck/bytedeck/issues/2167)
  - Hover text set up on a page did nothing unless that page also switched it on, so a question's marker notes (and other tooltips) stayed hidden; hover text now works everywhere [#2166](https://github.com/bytedeck/bytedeck/issues/2166)
  - An attached file's name was treated as HTML rather than as text, so a name containing markup could alter the page around it [#2169](https://github.com/bytedeck/bytedeck/issues/2169)
