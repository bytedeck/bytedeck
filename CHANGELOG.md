...
### [1.32.0] Unreleased Claude IV
* New Features:
  - **Quests can ask questions, answered as part of the submission.** Open a quest and use the new **Manage Questions** button (on the quest edit form beside Edit Prerequisites, and on the quest's own page) to give it as many questions as you like, of three kinds: a **short answer** (a single line, up to 200 characters, and the student is told the limit), a **long answer** (the full editor, with formatting and images), and a **file upload**, where you say what it accepts: an image, a video, an audio recording, an image or video, or any file at all. Each question carries the instructions the student reads, an optional **solution** and **marker notes** that only staff ever see, and a Required flag; the up and down arrows put the questions in the order students answer them, and the table reorders in place without reloading the page. The student then fills in one field per question above the usual comment box when they hand the quest in, with their typing saved as they go, and skipping a quest clears the answers they had started. When you mark it, each answer sits beside that question's solution and marker notes, so you can compare without leaving the page, and a photo, video or audio recording is shown or played right there rather than being a file to download. A quest with no questions behaves exactly as it always has, copying a quest copies its questions, and a quest shared to the Shared Library takes its questions with it [#1304](https://github.com/bytedeck/bytedeck/issues/1304) [#2161](https://github.com/bytedeck/bytedeck/issues/2161) [#2162](https://github.com/bytedeck/bytedeck/issues/2162) [#2172](https://github.com/bytedeck/bytedeck/issues/2172) [#2216](https://github.com/bytedeck/bytedeck/issues/2216) [#2347](https://github.com/bytedeck/bytedeck/issues/2347) [#2401](https://github.com/bytedeck/bytedeck/issues/2401)
  - **A deck can run more than one semester at once.** Each semester now has a stage of its own, shown in the semester list: **Upcoming** while you set it up, **Open** once you press Start, and **Archived** once final marks are recorded (an archived semester is read-only, so its dates and excluded days can no longer be edited out from under the marks that were calculated from them). Starting a semester no longer stops the one already running, so two course groups on different calendars can each run their own term, and each is archived on its own from its own row in the list. A student belongs to one semester through their course registration, and when more than one is open they choose which one they are joining; everything of theirs follows that registration rather than a deck-wide setting, so their XP, marks and progress chart cover their own term, and their teacher's approval queue holds their work. Between terms a deck can now sit with no semester open at all, and staff see one banner naming each semester that has ended, linking to the page that archives it; anyone joining or adding a student to a course while nothing is open is told that is why (staff with a link to the semesters page), and staff can still correct a registration whose semester has since been archived [#1177](https://github.com/bytedeck/bytedeck/issues/1177) [#1781](https://github.com/bytedeck/bytedeck/issues/1781) [#2157](https://github.com/bytedeck/bytedeck/issues/2157) [#2506](https://github.com/bytedeck/bytedeck/issues/2506) [#2507](https://github.com/bytedeck/bytedeck/issues/2507)
  - **Students say which course their XP counts toward.** A student in two courses used to have every quest's XP split evenly between them, so work done entirely in one course showed up as half of it in the other, and that even split was what went into their final mark. They are now asked, as they hand the quest in, which course it counts toward, and it counts there in full. Nobody who has no choice to make is asked: a student in one course, or a staff member commenting, sees no picker. The deck setting **students choose which course their XP counts toward** (Site Configuration) is on to begin with; turn it off and XP is shared evenly again, as it is today. A student in more than one course also gets their rank per course (the navbar icon opens a row for each) and a button per course above their XP chart, each line drawn against that course's own XP for 100% [#2440](https://github.com/bytedeck/bytedeck/issues/2440) [#2453](https://github.com/bytedeck/bytedeck/issues/2453)
  - **A course can run on XP alone, with no mark.** Each course form gains a **Use mark percentages** switch, on to begin with, so nothing changes until you turn it off for a course. With it off, that course's students have no mark rather than a meaningless one: the student list leaves their mark column blank, the marks page shows what they earned in XP instead of a percentage, and their XP chart drops its mark lines and percent axis. Graded and ungraded courses can run side by side on the same deck, and a student in both sees percentages only where they mean something [#403](https://github.com/bytedeck/bytedeck/issues/403)
  - **The Shared Library credits who shared each quest, and says what to do after an import.** Quest and campaign pages in the Library now name the teacher who shared the content and the deck it came from, recorded at the moment it is pushed, which is what sharing under CC BY-SA asks for. And an import used to end with "Successfully imported X to your deck", leaving out that the quest arrives unpublished and without a prerequisite, so it is invisible to students until you fix both: the message now names those two steps and links straight to the pages that perform them [#2377](https://github.com/bytedeck/bytedeck/issues/2377)
  - **A name you already use no longer blocks an import.** A quest whose name matches one of your own is renamed as it arrives instead of the import failing, so the starter content a shared library exists to distribute (two decks both having a "Welcome to ByteDeck!") imports cleanly. This matters most for campaigns: an import is all or nothing, so a single clashing name used to discard every other quest in it, and the bigger the campaign the likelier that was [#2364](https://github.com/bytedeck/bytedeck/issues/2364) [#2397](https://github.com/bytedeck/bytedeck/issues/2397)
  - **Searching the Library searches the whole Library.** The Quests tab now loads 15 at a time and searches in the database, matching a quest's name, its campaign's title or one of its tags, so a search finds everything shared rather than only what your browser had already downloaded [#2379](https://github.com/bytedeck/bytedeck/issues/2379)
  - **Sharing a quest tells you what will not travel with it.** A General Info block (shared text such as a marking rubric or how to record and submit a video) cannot cross into the Library, and neither can a prerequisite with no counterpart there. Both are now reported to you, the sharer, who is the only one who can act on them, rather than being an invisible loss the importing teacher discovers later [#2398](https://github.com/bytedeck/bytedeck/issues/2398) [#2442](https://github.com/bytedeck/bytedeck/issues/2442)
  - **Deck staff hear about a new ByteDeck release in the app.** When a new version reaches production, every deck's staff get a notification linking to the release announcement, so you find out what changed without watching GitHub [#2316](https://github.com/bytedeck/bytedeck/issues/2316)
  - **Rank icons are picked from a searchable list.** The rank add/edit form gains an icon picker, so you can find a Font Awesome icon by name and see it before saving instead of typing the class by hand. Existing ranks are converted as they are, and render unchanged [#2468](https://github.com/bytedeck/bytedeck/issues/2468)
* Tweaks:
...
* Bugfixes:
  ...
  - Archiving a student, banning them from comments, publishing an announcement, deleting a quest map and approving a quest all happened on following a plain link, so any page holding a crafted image could fire one of them on your behalf; they now go through a button that posts [#2383](https://github.com/bytedeck/bytedeck/issues/2383)
  - A TA could set the quest fields their form hides from them (published, editor, archived, available outside the course) by sending them directly [#2384](https://github.com/bytedeck/bytedeck/issues/2384)
  - A signed-in user could read another deck's quest content by naming that deck in a request, since the quest popup took the deck to read from the request itself [#2362](https://github.com/bytedeck/bytedeck/issues/2362)
  - A notification link could carry you off to any other site; it now only follows links back into your own deck [#2340](https://github.com/bytedeck/bytedeck/issues/2340)
  - One refused email address stopped a deck's whole nightly notification-digest batch, so everyone after it got nothing that day; a refusal now skips only that address and a throttled batch is retried a few minutes later [#2510](https://github.com/bytedeck/bytedeck/issues/2510)
  - Notifications rendered the names they mention as HTML, so a student who put markup in their own preferred name could run script for every teacher who opened their notifications; the names are now escaped, and the comment preview a notification shows is sanitized rather than trusted [#2511](https://github.com/bytedeck/bytedeck/issues/2511)
  - The pop-up messages the site shows after an action rendered stored text as HTML, so a quest named with a script tag could run script in the browser of anyone whose message mentioned that quest; messages are now escaped [#2498](https://github.com/bytedeck/bytedeck/issues/2498)
  - Turning the Shared Library off only hid its menu link: every Library page stayed reachable and an import still completed, content that had not been reviewed could be imported, and the export pages could be reached by teachers the Share buttons were never offered to [#2363](https://github.com/bytedeck/bytedeck/issues/2363) [#2365](https://github.com/bytedeck/bytedeck/issues/2365) [#2366](https://github.com/bytedeck/bytedeck/issues/2366) [#2368](https://github.com/bytedeck/bytedeck/issues/2368)
  - A push to the Shared Library that failed partway could leave part of a campaign in the Library while telling you the push had failed, and could email the Library's reviewers about content that was never written; a push now lands whole or not at all [#2372](https://github.com/bytedeck/bytedeck/issues/2372)
  - Importing a quest or campaign your deck already holds, or sharing one the Library already has, ended on a bare "permission denied" page: the message now names the content, links to your own copy when there is one, and says nothing was changed [#2373](https://github.com/bytedeck/bytedeck/issues/2373)
  - The email that tells Library staff content is awaiting review arrived as one run-together block, the content's name running straight into the sentence after it; it now has its paragraphs, and the site's usual email footer [#2371](https://github.com/bytedeck/bytedeck/issues/2371)
  - A shared quest dropped out of the Library's Quests tab the day the sharing deck's term ended, while its campaign went on offering it [#2449](https://github.com/bytedeck/bytedeck/issues/2449)
  - The page you decide a campaign import on was headed just "Campaign" and said the campaign had no description, whatever it actually said [#2370](https://github.com/bytedeck/bytedeck/issues/2370)
  - Library pages could show your own deck's content in place of the shared content they were describing, which was worst on the import confirmation pages, where it described something other than what you were about to import [#2369](https://github.com/bytedeck/bytedeck/issues/2369)
  - A quest imported from the Library could arrive wearing another deck's tags, an import that failed reached you as a 500 or a bare 404 rather than an explanation, and sharing a campaign holding a quest already in the Library could attach that copy to unrelated content [#1792](https://github.com/bytedeck/bytedeck/issues/1792) [#2367](https://github.com/bytedeck/bytedeck/issues/2367)
  - The mark distribution chart on a student's marks page compared them against the wrong list of classmates, and a single classmate whose mark had never been calculated made the page fail for everyone [#2157](https://github.com/bytedeck/bytedeck/issues/2157)
* Codebase:
  - Continued the push toward full test coverage, and closed the access checks that the tests being written turned up along the way [#2170](https://github.com/bytedeck/bytedeck/issues/2170) [#2296](https://github.com/bytedeck/bytedeck/issues/2296) [#2306](https://github.com/bytedeck/bytedeck/issues/2306) [#2321](https://github.com/bytedeck/bytedeck/issues/2321) [#2322](https://github.com/bytedeck/bytedeck/issues/2322) [#2327](https://github.com/bytedeck/bytedeck/issues/2327) [#2333](https://github.com/bytedeck/bytedeck/issues/2333) [#2334](https://github.com/bytedeck/bytedeck/issues/2334) [#2335](https://github.com/bytedeck/bytedeck/issues/2335) [#2342](https://github.com/bytedeck/bytedeck/issues/2342) [#2350](https://github.com/bytedeck/bytedeck/issues/2350) [#2352](https://github.com/bytedeck/bytedeck/issues/2352) [#2390](https://github.com/bytedeck/bytedeck/issues/2390) [#2400](https://github.com/bytedeck/bytedeck/issues/2400)
  - Rewrote how the Shared Library moves content: a quest is now copied from one deck's schema to the other directly, rather than being flattened into a spreadsheet of text cells on the way, which is what lost its tags and its prerequisite graph, and a copy that cannot complete now says so instead of looking like it worked [#2378](https://github.com/bytedeck/bytedeck/issues/2378) [#2380](https://github.com/bytedeck/bytedeck/issues/2380) [#2445](https://github.com/bytedeck/bytedeck/issues/2445)
  - Comment lists fetch in a fixed number of queries rather than one per comment, so a busy submission page loads faster [#2168](https://github.com/bytedeck/bytedeck/issues/2168)
  - Archiving a semester works out each student's per-course XP once rather than once per course they are in, and the XP progress chart is drawn from one pass over the student's work rather than one query per class day [#2459](https://github.com/bytedeck/bytedeck/issues/2459)
  - Removed the dead signal that rewrote Site domains [#2258](https://github.com/bytedeck/bytedeck/issues/2258)
  - Extended the contributor documentation that guides Claude Code sessions in this repository: building a real test environment rather than reading the code, replying on a review thread before resolving it, filing an issue for every suggestion left undone, watching a pull request on a timer instead of a stream of raw events, linking GitHub references in chat, saying which population each roster query means, requesting automated code reviews on a schedule that respects the shared rate limit, and labelling every issue they open [#2307](https://github.com/bytedeck/bytedeck/issues/2307) [#2311](https://github.com/bytedeck/bytedeck/issues/2311) [#2361](https://github.com/bytedeck/bytedeck/issues/2361) [#2407](https://github.com/bytedeck/bytedeck/issues/2407) [#2421](https://github.com/bytedeck/bytedeck/issues/2421) [#2434](https://github.com/bytedeck/bytedeck/issues/2434) [#2466](https://github.com/bytedeck/bytedeck/issues/2466) [#2472](https://github.com/bytedeck/bytedeck/issues/2472) [#2515](https://github.com/bytedeck/bytedeck/issues/2515) [#2517](https://github.com/bytedeck/bytedeck/issues/2517)
  - Filled the missing docstrings in the Shared Library app's code [#2406](https://github.com/bytedeck/bytedeck/issues/2406)
  - Took the routine dependency updates, including the Django 5.2 LTS security release, and made requirements.txt the single source of truth for the ruff version the hooks and CI use [#2323](https://github.com/bytedeck/bytedeck/issues/2323) [#2324](https://github.com/bytedeck/bytedeck/issues/2324) [#2337](https://github.com/bytedeck/bytedeck/issues/2337) [#2339](https://github.com/bytedeck/bytedeck/issues/2339) [#2348](https://github.com/bytedeck/bytedeck/issues/2348) [#2353](https://github.com/bytedeck/bytedeck/issues/2353) [#2354](https://github.com/bytedeck/bytedeck/issues/2354) [#2355](https://github.com/bytedeck/bytedeck/issues/2355) [#2391](https://github.com/bytedeck/bytedeck/issues/2391) [#2417](https://github.com/bytedeck/bytedeck/issues/2417) [#2418](https://github.com/bytedeck/bytedeck/issues/2418) [#2419](https://github.com/bytedeck/bytedeck/issues/2419) [#2422](https://github.com/bytedeck/bytedeck/issues/2422) [#2503](https://github.com/bytedeck/bytedeck/issues/2503)
* Devops:
  - Modernized the Docker and Compose setup the project develops and deploys with, and stopped hard-coding the production CloudFront domain [#2173](https://github.com/bytedeck/bytedeck/issues/2173) [#2260](https://github.com/bytedeck/bytedeck/issues/2260)
  - Bounded the worker and Redis memory, with a switch for the September scale-up, and made the deploy build on freshly pulled base images [#2082](https://github.com/bytedeck/bytedeck/issues/2082) [#2345](https://github.com/bytedeck/bytedeck/issues/2345)
  - The Subscription column in the deck admin sorts when you click it, listing decks by lifecycle [#2336](https://github.com/bytedeck/bytedeck/issues/2336)
  - On a fresh install, the seeded homepage's TRY IT buttons lead to the deck request form at `/decks/request/` (they pointed at a section that no longer exists), and the Shared Library deck is given a domain that resolves [#2312](https://github.com/bytedeck/bytedeck/issues/2312) [#2382](https://github.com/bytedeck/bytedeck/issues/2382)
  - A deck owner finished with their deck can request its deletion from the Subscription page instead of paying on or abandoning it against the 365-day clock: an operator reviews the request, lifecycle notices are muted while it stands, and the owner can withdraw it at any time [#2330](https://github.com/bytedeck/bytedeck/issues/2330)
  - A deck's Stripe customer-portal configuration id is stored on the deck itself rather than in a 30-day cache, so an evicted cache no longer piles up duplicate portal configurations in the Stripe account; clearing the field in the tenant admin rebuilds it from the account default [#2465](https://github.com/bytedeck/bytedeck/issues/2465)
  - Documented where each Stripe value in the env examples comes from, and taught the release workflow to announce only the changelog sections it has not announced before [#2297](https://github.com/bytedeck/bytedeck/issues/2297) [#2305](https://github.com/bytedeck/bytedeck/issues/2305)
