...
* Claude Code: stay quiet in the chat while watching a PR unless something actually matters. Self-check-in wakeups, routine webhook events (CI still running, Codecov/CodeRabbit informational comments, a "review in progress" placeholder, "nothing changed"), and silent re-arms must **not** produce a user-facing chat message. When a check-in fires or an event arrives and there's nothing actionable, do the work silently (re-check state, re-arm the next check-in) and end the turn without posting. Only write to the chat when there's something the user genuinely needs: a decision or answer you need from them, a real CI failure you're diagnosing or fixing, a review finding you're acting on or declining, a blocker, or a terminal outcome (a merge/close, or a finished deliverable). The goal: when the user opens the chat after a while, it holds decisions and real content, not a wall of "still green, re-armed" status lines. This governs only user-facing chat text: keep thinking and running tools (checking the PR, re-arming the timer) as needed.
...
* Claude Code: CodeRabbit casts real review states (Approve / Request changes): the automated CodeRabbit review runs with `request_changes_workflow: true` (`.coderabbit.yaml`), so it submits a GitHub **Request changes** review while it has open findings and **Approves** once they're resolved (or immediately on a clean PR). Its approval counts toward the repo's "Require 1 approval" branch-protection rule, so **either CodeRabbit's approval or a developer's satisfies the review requirement**. This only *informs* the review status; the maintainer still performs the actual merge (no auto-merge). Treat its findings as review comments to clear, not a hard wall: act on the good ones (push the fix; it re-reviews the new commit and flips Request-changes → Approve, then `resolve_review_thread`), reply on the ones you're declining with a short reason (then resolve the thread, or the maintainer dismisses its review). You can steer it with `@coderabbitai review` / `resolve` / `pause` comments. Its walkthrough/summary comments are informational (no action needed).
  * **If CodeRabbit is rate-limited** (it posts a "Review limit reached / Next review available in N minutes" notice *instead of* reviewing, so the PR has had no CodeRabbit pass), don't leave it un-reviewed: schedule a reminder (e.g. `send_later` for a few minutes past that stated window) and, once the cooldown has elapsed, post an `@coderabbitai review` comment on the PR to trigger the review. Don't fire it before the window (it just re-hits the limit and wastes quota); a natural new push to the branch also re-triggers a review, so if you're pushing a fix after the cooldown you don't need the separate comment.
