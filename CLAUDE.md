...
* Claude Code: sign off everything you post to GitHub (PR descriptions, comments, review comments, issues) with "- Claude Code (`<session-id>`)", where `<session-id>` is this session's id — the `session_…` value (e.g. `session_01BMPt54dgApiXjk6TXqD2fi`), the same id in the `https://claude.ai/code/<session-id>` link auto-added to PR descriptions. Including the session id lets the maintainer reply in the exact session that made the post.
