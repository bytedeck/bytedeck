...
def quest_list(request, quest_id=None, template='quest_manager/quests.html'):
    ...
    available_quests_count = len(available_quests) if isinstance(available_quests, list) else available_quests.count()
        ...
        if form.is_valid():
            ...
            if badges:
                for badge in badges:
                    ...
                    comment_text_addition += (
                        '<p></br>' + rarity_icon + 'The <b>' + badge.name + '</b> badge was granted for this quest ' + rarity_icon + '</p>'
                    )
            ...
            if 'approve_button' in request.POST:
                ...
                note_verb = 'skipped'
                icon = "<span class='fa-stack text-muted'>" + "<i class='fa fa-shield fa-stack-1x'></i>" + '</span>'
                blank_comment_text = '<p>(Skipped - You were not granted XP for this quest)</p>'
...
def hide(request, quest_id):
    ...
    messages.warning(
        request,
        '<strong>' + quest.name + '</strong> has been added to your list of hidden quests.',
    )
...
def unhide(request, quest_id):
    ...
    messages.success(
        request,
        '<strong>' + quest.name + '</strong> has been removed from your list of hidden quests.',
    )
...
def submission(request, submission_id=None, quest_id=None):
    ...
    if request.user.is_staff:
        ...
        if not draft_comment:
            ...
            if sub.draft_text:
                text = sub.draft_text
            draft_comment = Comment.objects.create_comment(user=request.user, path=sub.get_absolute_url(), text=text, target=None)
...
def unflag(request, submission_id):
    ...
    messages.success(
        request,
        "Submission <a href='%s'>%s by %s</a> has been unflagged." % (sub.get_absolute_url(), sub.quest_name(), sub.user),
    )
