...
def publish_selected_quests(modeladmin, request, queryset):
    ...
    msg_str = f'{str(num_updates)} quest(s) updated. Editors have been removed and the quest is now visible to students.'  # noqa
...
class QuestSubmissionAdmin(NonPublicSchemaOnlyAdminAccessMixin, admin.ModelAdmin):
    ...
    def get_queryset(self, request):
        qs = QuestSubmission.objects.get_queryset(
            active_semester_only=False, exclude_archived_quests=False, exclude_quests_not_visible_to_students=False
        )
