...
class SubmissionViewTests(TenantTestCase):
    ...
    def test_comment_and_questions_deleted_on_drop(self):
        ...
        We will also ensure acknowledge that since submitted comments aren't deleted when the
        quest submission is, the questionsubmissions won't be either.
        """
        self.client.force_login(self.test_student1)

        # start the quest (create a submission)
        current_comment = baker.make(Comment, text="I am a test draft comment")
        new_sub = baker.make(QuestSubmission, quest=self.quest1, user=self.test_student1,
        ...
        previous_question_subs = [baker.make(QuestionSubmission, question=question, comment=previous_comment)
                                  for question in self.quest1.question_set.all()]
        ...
        # acknowledge that the previous QuestionSubmissions were not deleted
        self.assertTrue(Comment.objects.filter(id=previous_comment.id).exists())
        for question_sub in previous_question_subs:
            self.assertTrue(QuestionSubmission.objects.filter(id=question_sub.id).exists())
