...
class RestrictedFileField(models.FileField):
    ...
    * max_upload_size - a number indicating the maximum file size allowed for upload.
    ...
    def formfield(self, **kwargs):
        return super().formfield(form_class=RestrictedFileFormField, max_upload_size=self.max_upload_size, content_types=self.content_types)
...
class MenuItem(models.Model):
    ...
    def __str__(self):
        ...
        return f'<a href="{self.url}" {target} class="menuitem">' f'<i class="fa fa-fw fa-{self.fa_icon}"></i>&nbsp;&nbsp;{self.label}' '</a>'
