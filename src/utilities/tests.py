...
class FlatPageViewTests(ViewTestUtilsMixin, TenantTestCase):
    ...
    def test_all_page_status_codes_for_anonymous(self):
        ...
        """
        self.assert200('utilities:flatpage-list')

        # Staff only
        self.assertRedirectsAdmin('utilities:flatpage-create')
        self.assertRedirectsAdmin('utilities:flatpage-edit', args=[1])
        self.assertRedirectsAdmin('utilities:flatpage-delete', args=[1])
    ...
    def test_all_page_status_codes_for_staff(self):
        ...
        self.assert200('utilities:flatpage-list')
        self.assert200('utilities:flatpage-create')
        self.assert200('utilities:flatpage-edit', args=[pk])
        self.assert200('utilities:flatpage-delete', args=[pk])
    ...
    def test_login_requirements_for_flatpage(self):
        ...
        # check all exists in flatpage-list first
        response = self.client.get(reverse('utilities:flatpage-list'))
        ...
        for flatpage in self.flatpage_nonlogin:
            self.assert200('django.contrib.flatpages.views.flatpage', kwargs={'url': flatpage.url})

        # login requried
        for flatpage in self.flatpage_login:
            self.assertRedirectsLogin('django.contrib.flatpages.views.flatpage', kwargs={'url': flatpage.url})
    ...
    def test_flatpagecreate__create(self):
        ...
        response = self.client.post(reverse('utilities:flatpage-create'), data=data)
        self.assertRedirects(response, reverse('utilities:flatpage-list'))

        # Shows up in flatpage-list
        response = self.client.get(reverse('utilities:flatpage-list'))
        self.assertContains(response, data['title'])

        # Accessed in pages/
        self.assert200('django.contrib.flatpages.views.flatpage', kwargs={'url': data['url']})
    ...
    def test_flatpageupdate__update(self):
        ...
        response = self.client.get(reverse('django.contrib.flatpages.views.flatpage', kwargs={'url': pre_update_data['url']}))
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, pre_update_data['title'])
        self.assertContains(response, pre_update_data['content'])

        # update through view
        response = self.client.post(reverse('utilities:flatpage-edit', args=[flatpage.pk]), data=post_update_data)
        self.assertRedirects(response, reverse('django.contrib.flatpages.views.flatpage', kwargs={'url': post_update_data['url']}).replace('//', '/'))

        # Check post-update variables are correct
        response = self.client.get(reverse('utilities:flatpage-list'))
        self.assertContains(response, post_update_data['title'])

        response = self.client.get(reverse('django.contrib.flatpages.views.flatpage', kwargs={'url': post_update_data['url']}))
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, post_update_data['title'])
    ...
    def test_flatpagedelete__delete(self):
        ...
        # delete through view
        response = self.client.post(reverse('utilities:flatpage-delete', kwargs={'pk': pk}))
        self.assertRedirects(response, reverse('utilities:flatpage-list'))

        # check if deleted

        # doesnt exists in flatpage-list
        response = self.client.get(reverse('utilities:flatpage-list'))
        self.assertNotContains(response, title)

        # page does not exist
        self.assert404('django.contrib.flatpages.views.flatpage', kwargs={'url': url})
