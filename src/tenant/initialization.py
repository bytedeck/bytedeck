...
def load_orientation_campaign():
    ...
    # Check that the "Orientation" campaign exits (that was created in last migration)
    # might not exists for a non-new tenant with existing data
    if Category.objects.filter(title="Orientation").exists():  
        orientation_campaign = Category.objects.get(title="Orientation")
    else:
        orientation_campaign = None

    # if there is only one quest and it's our welcome quest, then this is a new tenant and we can add the new quests
    if orientation_campaign and Quest.objects.count() == 1 and Quest.objects.filter(import_id="bee53060-c332-4f75-85e1-6a8f9503ebe1").exists():
        ...
        contract_quest = Quest.objects.create(
            ...
            xp=2,
            icon="icons/thumb_386444_Contract.png",
        ...
        # now link them with pre-requisites:        
        ...
        if Badge.objects.filter(name='ByteDeck Proficiency').exists():
            proficiency_badge = Badge.objects.get(name='ByteDeck Proficiency')
        
            Prereq.add_simple_prereq(proficiency_badge, avatar_quest)
            Prereq.add_simple_prereq(proficiency_badge, screenshots_quest)
            Prereq.add_simple_prereq(proficiency_badge, contract_quest)
            Prereq.add_simple_prereq(proficiency_badge, cc_quest)
