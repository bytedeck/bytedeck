...
from hackerspace_online.settings import sys
...
def get_root_url_for_initialization():
    """get root url to append static urls to when locating static files to upload. clause in place to prevent weird bug while running tests..."""
    if "test" in sys.argv:
        root_url = "http://testing.localhost:8000"
    else:
        root_url = Tenant.get().get_root_url()
    return(root_url)
...
def create_initial_badges():
    ...
    # upload static file icons programmatically to bypass an error for developers that breaks all custom icons assigned in creation
    # iterate through created awards and upload their icons from static folder according to name
    badgenames = ["Penny", "Nickel", "Dime"]
    root_url = get_root_url_for_initialization()
    for badgename in badgenames:
        badge = Badge.objects.get(name=badgename)
        static_url = static("badges/img/" + badgename + ".png")
        img_temp = tempfile.NamedTemporaryFile()
        with urlopen(root_url + static_url) as response:
            img_temp.write(response.read())
        img_temp.flush()
        badge.icon.save(badgename + ".png", ImageFile(img_temp))
    ...
    # team badges need programmatically assigned icons as well, but need individual assignments because static filenames differ from display names.
    badgenames = ["Red Team", "Green Team", "Blue Team"]
    for badgename in badgenames:
        badge = Badge.objects.get(name=badgename)
        static_url = static("badges/img/" + badgename.replace(" ", "_") + ".png")  # static filepath for teams needs an underscore instead of a space
        img_temp = tempfile.NamedTemporaryFile()
        with urlopen(root_url + static_url) as response:  # root_url defined earlier in function
            img_temp.write(response.read())
        img_temp.flush()
        badge.icon.save(badgename + ".png", File(img_temp))
...
def create_orientation_campaign():
    ...
    # quests have names that aren't very transferrable to icon filenames so assignment must happen individually.
    # bytedeck class contract quest
    root_url = get_root_url_for_initialization()
    quest = Quest.objects.get(name="ByteDeck Class Contract")
    static_url = static("quest_manager/img/contract.png")
    img_temp = tempfile.NamedTemporaryFile()
    with urlopen(root_url + static_url) as response:
        img_temp.write(response.read())
    img_temp.flush()
    quest.icon.save("contract.png", File(img_temp))

    # create an avatar quest
    quest = Quest.objects.get(name="Create an Avatar")
    static_url = static("quest_manager/img/myAvatar_1.png")
    img_temp = tempfile.NamedTemporaryFile()
    with urlopen(root_url + static_url) as response:
        img_temp.write(response.read())
    img_temp.flush()
    quest.icon.save("myAvatar_1.png", File(img_temp))

    # who owns your creations 
    quest = Quest.objects.get(name="Who owns your creations?")
    static_url = static("quest_manager/img/cc.large.png")
    img_temp = tempfile.NamedTemporaryFile()
    with urlopen(root_url + static_url) as response:
        img_temp.write(response.read())
    img_temp.flush()
    quest.icon.save("cc.large.png", File(img_temp))
    ...
    proficiency_badge.add_simple_prereqs([avatar_quest, screenshots_quest, contract_quest, cc_quest])
