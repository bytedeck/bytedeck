...
class AnnouncementTasksTests(TenantTestCase):
    ...
    def test_get_users_to_email(self):
        ...
        for i in range(11):
            ...
            if not is_staff:
                baker.make(CourseStudent, user=user, course=course, semester=semester)
    ...
    def test_send_announcement_emails(self):
        task_result = tasks.send_announcement_emails.apply(
            kwargs={'content': '', 'root_url': 'https://example.com', 'absolute_url': '/link/to/announcement/'}
        )
    ...
    def test_publish_announcement(self):
        ...
        task_result = tasks.publish_announcement.apply(
            kwargs={'user_id': self.test_teacher.id, 'announcement_id': self.announcement.id, 'root_url': 'https://example.com'}
        )
        self.assertTrue(task_result.successful())
    ...
    def test_publish_announcement_past_date(self):
        past_announcement = baker.make(Announcement, datetime_released=timezone.now() - timedelta(days=90))
        self.assertTrue(past_announcement.draft)
        self.assertFalse(past_announcement.auto_publish)

        task_result = tasks.publish_announcement.apply(
            kwargs={'user_id': self.test_teacher.id, 'announcement_id': past_announcement.id, 'root_url': 'https://example.com'}
        )
