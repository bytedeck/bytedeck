...
def generate_test_png_file():
    ...
    uploaded_file = InMemoryUploadedFile(file=output, field_name=None, name='minimal.png', content_type='image/png', size=output.tell(), charset=None)
...
class PortfolioViewTests(ViewTestUtilsMixin, TenantTestCase):
    ...
    def test_UpdateView__post(self):
        ...
        # post request test
        response = self.client.post(reverse('portfolios:edit', args=[self.portfolio.pk]), data=form_data)
    ...
    def test_ArtworkCreateView__post(self):
        ...
        response = self.client.post(reverse('portfolios:art_create', args=[self.portfolio.pk]), data=form_data)
        form = response.context['form']
