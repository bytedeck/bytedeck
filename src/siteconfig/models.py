...
def get_default_deck_owner():
    ...
    """ 
    # return default owner if it exists. This will run after initialization.py runs
    owner_user_qs = User.objects.filter(username=settings.TENANT_DEFAULT_OWNER_USERNAME, is_superuser=True, is_staff=True)
    if owner_user_qs.exists():
        return owner_user_qs.first().pk
    ...
    if non_admin_staff_qs.exists():  # i.e. there are more staff than just the admin user
        return non_admin_staff_qs.first().id  # return the first one (assumes sorted by id, so should be the oldest one?)
